...
6. Класс `OrdersDelivery` - наследуется от класса `Form`. Отображает форму оформления заказа с информацией о способе оплаты и адреса доставки, задаёт:
  ...
  #### Методы:
    ...
    `set payment` - используется для добавления способа оплаты,

    `set address` - используется для добавления адресса.

7. Класс `OrdersContacts` - наследуется от класса `Form`. Отображает форму оформления заказа с контактной информацией, задаёт:
